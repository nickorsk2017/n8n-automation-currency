...
#
# This script only imports (upserts) the workflow row; it never activates.
# n8n's CLI import never honors an `active` field in the JSON regardless, so
# activation is always a separate, explicit step — see scripts/activate_workflow.sh.
# Splitting the two lets `make import-all` import every file (in any order)
# before activating any of them, which is required when one workflow's
# activation depends on another workflow already existing (see
# scripts/order_workflows.py).
#
# Usage:
